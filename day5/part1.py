...
def make_moves(stack, moves):
    for move in moves:
        operators = [int(s) for s in move.split() if s.isdigit()]
        for count in range(0, operators[0]):
            moved_value = stack[operators[1] - 1].pop()
            stack[operators[2] - 1].append(moved_value)
    return stack


def calculate(lines):
    stack = build_stack_struct(lines[0:9])
    output_stack_struct = make_moves(stack, lines[10:])
    output = ""
