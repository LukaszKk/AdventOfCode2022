...
def index_all_2d(rlist, value):
    for y, y_list in enumerate(rlist):
        for x, x_element in enumerate(y_list):
            if value in x_element:
                yield y, x
...
def calculate(lines):
    ...
    all_path = []
    for start_a in index_all_2d(heightmap, "a"):
        all_path.append(find_path(heightmap, start_a, end))
    distances = [len(x) for x in all_path if len(x) > 0]
    end_time = time.time()
